...
// Sandbox schedules — recurring scan schedules per sandbox
export const sandboxSchedules = mysqlTable("sandbox_schedules", {
  id: int("id").autoincrement().primaryKey(),
  sandboxId: int("sandbox_id").notNull(),
  userId: int("user_id"),
...
export type SandboxSchedule = typeof sandboxSchedules.$inferSelect;
export type InsertSandboxSchedule = typeof sandboxSchedules.$inferInsert;
...
// ─── Meta Ads CAPI ────────────────────────────────────────────────────────────
// Meta pixel configurations per user
export const metaPixels = mysqlTable("meta_pixels", {
  id: int("id").autoincrement().primaryKey(),
  userId: int("user_id").notNull(),
  name: varchar("name", { length: 128 }).notNull(),
  pixelId: varchar("pixel_id", { length: 64 }).notNull(),
...
export type InsertMetaPixel = typeof metaPixels.$inferInsert;

// Meta CAPI event log
export const metaEvents = mysqlTable("meta_events", {
  id: int("id").autoincrement().primaryKey(),
  pixelId: int("pixel_id").notNull(),
  userId: int("user_id").notNull(),
